...
const DataRow = ({ label, value }: { label: string; value: string }) => (
  <div className="flex flex-wrap items-start gap-1 py-2 border-b border-slate-700/50 last:border-0">
    ...
    <span className="text-slate-200 text-sm flex-1 leading-relaxed">{value}</span>
...
const CertificateModal = ({
  ...
  return (
    <div
      className="sticky inset-0 z-99 flex items-center justify-center p-4"
      onClick={onClose}
    >
      {/* Backdrop */}
...
const CertificateCard = ({
  ...
  const topicList = cert.Topics.split(",").map((t) => t.trim());
      ...
      <div className="p-5 space-y-4">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="space-y-1 flex-1 min-w-0">
            ...
            <p className="text-amber-400/80 text-sm font-medium">{cert.Institute}</p>
          </div>
          <div className="flex flex-col items-end gap-1.5 shrink-0">
            <span className="text-xs font-bold bg-amber-500/10 text-amber-400 border border-amber-500/20
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          {topicList.length > 5 && (
            <span className="text-xs px-2 py-0.5 rounded-md bg-slate-700/40 text-slate-500 border border-slate-600/30">
        ...
        <div className="flex items-center justify-between pt-1 border-t border-slate-700/40">
          <span className="text-xs text-slate-500 font-mono">ID: {cert.FileId.slice(0, 12)}…</span>
          <span className="text-xs text-amber-400/70 group-hover:text-amber-400 transition-colors
...
const Certifications = () => {
  ...
  const [selectedCert, setSelectedCert] = useState<CertificatesRow | null>(null);

  const filtered = useMemo(() => {
    const q = search.toLowerCase();
    return (Certificates as CertificatesRow[]).filter(
      (c) =>
    ...
    <section className="min-h-screen bg-slate-950 px-4 py-12 font-sans">
      ...
      <div className="max-w-4xl mx-auto mb-8">
        <div className="relative">
          <svg className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500"
          ...
          <input
            type="text"
            placeholder="Search by title, institute, topic, or year…"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="w-full pl-11 pr-4 py-3 rounded-xl bg-slate-800/80 border border-slate-700/60
              text-slate-200 placeholder-slate-500 text-sm focus:outline-none focus:border-amber-500/50
        ...
        {filtered.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center space-y-3">
            ...
            <button onClick={() => setSearch("")}
              className="text-amber-400 text-xs hover:underline">Clear search</button>
          </div>
        )}
      </div>
